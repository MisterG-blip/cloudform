...
class SceneRenderer {
  ...
  _drawScreen(screen, offsetX, inventory, usedHotspots, consumedItems, deltaTime) {
    ...
    for (const obj of screen.objects || []) {
      ...
      // Glow — immer in voller Canvas-Größe (PNG hat gleiche Proportionen wie Hintergrund)
      if (obj.glow?.frames && this.hintAlpha > 0) {
        const frames  = obj.glow.frames;
        const glowSrc = frames[this.glowFrame % frames.length];
