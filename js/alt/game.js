...
class Game {
  constructor() {
    ...
    this.character.loadSprite('assets/character/maybel_sprite.png');
  ...
  _handleClick(x, y) {
    ...
    if (x >= hb.x && x <= hb.x + hb.w && y >= hb.y && y <= hb.y + hb.h) {
      this.sceneRenderer.triggerHint();
      return;
    }
  ...
  update(deltaTime) {
    ...
    if (this.character.walking) {
      this.audio?.startSteps(stepsSrc);
    } else {
  ...
  async start() {
    ...
    // Audio initialisieren — Musik wird vom Intro gesteuert
    ...
    if (!loaded) {
      ...
      this.intro.start(() => {
        if (MUSIC_SRC) this.audio.crossfadeMusic(MUSIC_SRC, 2000);
