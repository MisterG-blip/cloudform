...
const STATES = {
  ...
  blink:      { sheet: SPRITE_IDLE, row: 0, frames: 1, fps: 1, startFrame: 1 },
  smile:      { sheet: SPRITE_IDLE, row: 0, frames: 1, fps: 1, startFrame: 2 },
  lookright:  { sheet: SPRITE_IDLE, row: 0, frames: 1, fps: 1, startFrame: 3 },
  idle_chew:  { sheet: SPRITE_IDLE, row: 1, frames: 4, fps: 6, startFrame: 0 },
  idle_gum:   { sheet: SPRITE_IDLE, row: 2, frames: 5, fps: 6, startFrame: 0 },
  walk_down:  { sheet: SPRITE_WALK, row: 0, frames: 4, fps: 6 },
  walk_right: { sheet: SPRITE_WALK, row: 1, frames: 4, fps: 6 },
  walk_up:    { sheet: SPRITE_WALK, row: 2, frames: 4, fps: 6 },
  walk_left:  { sheet: SPRITE_WALK, row: 3, frames: 4, fps: 6 },
};
...
class Character {
  ...
  loadSprite() {
    const load = (src, key) => {
      ...
      img.onerror = () => { this[key] = null; console.warn('Character sprite not found:', src); };
      img.src = src;
    };
    load(SPRITE_IDLE, '_imgIdle');
    load(SPRITE_WALK, '_imgWalk');
  }
  ...
  draw(ctx) {
    ...
    if (this._imgIdle || this._imgWalk) {
      const img = cfg.sheet === SPRITE_WALK ? this._imgWalk : this._imgIdle;
      if (img) {
